...
macro_rules! wrapped_class_definition {
    ...
    { $cls:ident ; $($initbody:tt)* ; ($($mimpl:tt)*) ; ($($mdef:tt)*) ; def $name:ident( $self_arg:tt , $($arg:ident : $argty:ty),* ) -> $ret:ty $body:block $($rest:tt)* } => {
    ...
    { $cls:ident ; $($initbody:tt)* ; ($($mimpl:tt)*) ; ($($mdef:tt)*) ; def $name:ident( $self_arg:tt ) -> $ret:ty $body:block $($rest:tt)* } => {
